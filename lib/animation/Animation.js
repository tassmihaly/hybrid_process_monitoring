...
Animation.prototype._createTokenGfx = function(group, scope) {
  ...
  svgAttr(parent, { class: 'bts-token' });
  ...
  svgAppendTo(circle, parent);
  svgAppendTo(text, parent);
...
Animation.prototype._getGroup = function(scope) {
  ...
  if (!group) {
    group = svgCreate('g');
    svgAttr(group, { class: 'bts-animation-tokens' });

    // Only append if both are valid DOM Elements
    if (
      group instanceof window.SVGElement &&
      layer instanceof window.SVGElement
    ) {
      svgAppendTo(group, layer);
    } else {
      // Optionally log or handle the error
      console.warn('Animation: Not appending group, invalid DOM elements:', group, layer);
      return null;
    }
  }
