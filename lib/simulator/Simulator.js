...
export default function Simulator(injector, eventBus, elementRegistry) {
  ...
  function trigger(context) {
    ...
    const event = getEvent(_event);
      ...
      if (event && scope) {
        const isStartLike = (event.element && is(event.element, 'bpmn:StartEvent')) || event.type === 'none';
        if (isStartLike) {
          if (poolStarted.has(scope.element.id)) {
            return [];
          }
          poolStarted.add(scope.element.id);
        }
      }
    ...
    if (!interrupting.length) {
      if (!nonInterrupting.length && event.type === 'message') {
        const pending = findSet(pendingEvents, pending => {
          return pending.scope === scope && eventsMatch(event, pending.event);
        });
  ...
  function subscribe(scope, event, triggerFn) {
    ...
    if (event.type === 'message') {
      const pending = findSet(pendingEvents, pending => {
        return pending.scope === scope && eventsMatch(pending.event, event);
      });
  ...
  function destroyScope(scope, initiator = null) {
    ...
    if (scope.destroyed) {
      ...
      // remove pending events for this scope
  ...
  function reset() {
    // Ensure per-pool start tracking resets for a new simulation
    poolStarted.clear();
  ...
  this.pauseAllActivities = pauseAllActivities;
  this.unpauseAllActivities = unpauseAllActivities;
  ...
  // Add methods to pause/unpause all activities globally
  function pauseAllActivities() {
    elementRegistry.forEach(element => {
      if (isActivity(element)) {
        waitAtElement(element, true);
        elementChanged(element); // Trigger visual updates for this element
      }
    });
  }
