import {
  DFA,
  buildConstraintDFAs,
  convertBpmnToPetriNet,
  convertPetriNetToReachabilityGraph,
  mergeDFAs
} from './bpmn_utils';
...
export async function test(bpmnjs) {
  ...
  for (let i = 0; i < bpmnsByPools.length; i++) {
    ...
    console.log(graph);
    processDfas.push(DFA.from(graph));
  }

  const mergedProcessDfa = mergeDFAs(processDfas);
  console.log('[Frontend DFA] merged process DFA', mergedProcessDfa);

  const constraintDfas = buildConstraintDFAs(constrains, mergedProcessDfa.alphabet);
  let mergedFrontendDfa = mergedProcessDfa;

  for (let i = 0; i < constraintDfas.length; i++) {
    mergedFrontendDfa = mergedFrontendDfa.add_constraint(constraintDfas[i]);
  }

  mergedFrontendDfa = mergedFrontendDfa.addGlobalColor();

  console.log('[Frontend DFA] merged hybrid DFA', mergedFrontendDfa);

  const frontendDfa = parseColoredDFA(mergedFrontendDfa.toObject());
  globalColoredDFA = frontendDfa;
  currentDFAState = frontendDfa.initState;
...
function safeSerializeXml(xmlNodeOrDoc) {
  if (!xmlNodeOrDoc) {
    throw new Error('safeSerializeXml received an empty XML value');
  }

  const serializer = new XMLSerializer();

  try {
    return serializer.serializeToString(xmlNodeOrDoc);
  } catch (error) {
    if (typeof xmlNodeOrDoc === 'string') {
      return xmlNodeOrDoc;
    }

    if (xmlNodeOrDoc.documentElement) {
      try {
        return serializer.serializeToString(xmlNodeOrDoc.documentElement);
      } catch (_) {
        // Fall through to other string fallbacks.
      }
    }

    if (typeof xmlNodeOrDoc.xml === 'string') {
      return xmlNodeOrDoc.xml;
    }

    if (typeof xmlNodeOrDoc.outerHTML === 'string') {
      return xmlNodeOrDoc.outerHTML;
    }

    if (typeof xmlNodeOrDoc.toString === 'function') {
      const asString = xmlNodeOrDoc.toString();
...
function splitBpmnByPool(xmlDoc) {
  const doc = typeof xmlDoc === 'string' ? parseXml(xmlDoc) : xmlDoc;

  if (!doc || typeof doc.getElementsByTagName !== 'function') {
    throw new Error('splitBpmnByPool expects a parsed BPMN XML document or XML string');
  }

  const participants = [
    ...Array.from(doc.getElementsByTagName('bpmn:participant')),
    ...Array.from(doc.getElementsByTagName('participant'))
  ];
  const processes = [
    ...Array.from(doc.getElementsByTagName('bpmn:process')),
    ...Array.from(doc.getElementsByTagName('process'))
  ];

  if (participants.length === 0) {
    const processId = processes.length > 0 ? processes[0].getAttribute('id') : 'no_pool';
    const xmlStr = safeSerializeXml(doc);
  ...
  for (const participant of participants) {
    ...
    const defsTemplate = `<bpmn:definitions xmlns:bpmn="${bpmnNs}" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" id="Definitions_${processRef}" targetNamespace="http://bpmn.io/schema/bpmn"></bpmn:definitions>`;
    const newDefs = parseXml(defsTemplate);

    const defsElement = newDefs.documentElement;

    // Import the process into the new document
    const importedProcess = newDefs.importNode(originalProcess, true);
    defsElement.appendChild(importedProcess);

    // Serialize the new document
    const nodeToSerialize = newDefs && typeof newDefs.nodeType === 'number' ? newDefs : defsElement;
    const xmlStr = safeSerializeXml(nodeToSerialize);
...
function parseColoredDFA(coloredDfaData) {
  ...
  function parseTransitions(transitionData) {
    ...
    Object.keys(transitionData).forEach(state => {
      const stateTransitions = Array.isArray(transitionData[state]) ? transitionData[state] : [];
  ...
  function parseGlobalColors(globalColorsData) {
    const globalColors = {};

    if (!globalColorsData || typeof globalColorsData !== 'object') {
      return globalColors;
    }

    Object.keys(globalColorsData).forEach(state => {
      globalColors[state] = globalColorsData[state];
    });
  ...
  const parsedDFA = {
    ...
    globalColors: parseGlobalColors(coloredDfaData.globalColor),
    ...
    getStateGlobalColor: function(state) {
      const stateKey = typeof state === 'object' ? `(${state.join(',')})` : state;
      return this.globalColors[stateKey] || null;
    },
...
export function initializeSimulationMonitoring(bpmnjs) {
  ...
  try {
    ...
    useGlobalColorMode = Boolean(bpmnjs.__useGlobalColorMode);
    ...
    const toggleModeListener = (event) => {
      ...
      } else if (simulationEventBus) {
        simulationEventBus.fire('tokenSimulation.constraintColorsReset', {
          reason: 'simulation-deactivated'
        });
      }
    };
    ...
    const colorModeListener = (event = {}) => {
      useGlobalColorMode = Boolean(event.useGlobalColorMode);
      console.log('Constraint color mode:', useGlobalColorMode ? 'global' : 'local');

      // Repaint constraints immediately when toggling Local/Global mode.
      if (globalColoredDFA && currentDFAState) {
        const stateColor = globalColoredDFA.getStateColor(currentDFAState);
        publishConstraintColorsForState(currentDFAState, stateColor, 'mode-toggle');
      }
    };
    simulationEventBus.on(COLOR_MODE_CHANGED_EVENT, colorModeListener);
    eventListeners.push({ event: COLOR_MODE_CHANGED_EVENT, handler: colorModeListener });
    ...
    const resetSimulationListener = () => {
      ...
      if (globalColoredDFA) {
        ...
        const stateColor = globalColoredDFA.getStateColor(currentDFAState);
        publishConstraintColorsForState(currentDFAState, stateColor, 'reset');
      }
    };
...
function checkConstraintViolations(previousState, currentState, activityId, stateColor) {
  ...
  publishConstraintColorsForState(currentState, stateColor, activityId);
...
function publishConstraintColorsForState(currentState, stateColor, activityId) {
  if (!stateColor) {
    return;
  }

  const constraintFlowIds = getConstraintFlowIdsFromStateColor(stateColor);

  if (useGlobalColorMode) {
    const globalColor = globalColoredDFA.getStateGlobalColor(currentState);
    const globalStatus = globalColor && typeof globalColor === 'object' ? globalColor.global : null;

    if (!globalStatus) {
      return;
    }

    const targetConstraintFlowIds = constraintFlowIds.length > 0
      ? constraintFlowIds
      : (Array.isArray(globalConstraints) ? globalConstraints.map(constraint => constraint.id) : []);

    targetConstraintFlowIds.forEach(constraintFlowId => {
      publishConstraintStatusEvent(constraintFlowId, globalStatus, activityId, currentState);
    });

    return;
  }

  // Local mode: publish each constraint's own status.
  if (Array.isArray(stateColor)) {
    stateColor.forEach((constraintColor, index) => {
      if (constraintColor && typeof constraintColor === 'object') {
        Object.keys(constraintColor).forEach(constraintFlowId => {
...
function getConstraintFlowIdsFromStateColor(stateColor) {
  const ids = new Set();
  ...
  stateColor.forEach((constraintColor) => {
    if (!constraintColor || typeof constraintColor !== 'object') {
      return;
    }

    Object.keys(constraintColor).forEach(constraintFlowId => {
      if (constraintFlowId !== 'global') {
        ids.add(constraintFlowId);
      }
    });
  });
